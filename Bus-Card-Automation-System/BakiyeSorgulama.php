...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $pdo->prepare("SELECT id, balance FROM users WHERE idNumber = :idNumber");
    ...
    if ($user) {
        ...
        $stmt_log->execute([
            ':user_id' => $user['id'],
            ':islem_turu' => 'sorgulama',
            ':islem_tarihi' => $createdate,
            ':detay' => 'Bakiye sorgulandı.'
        ]);


        echo "<script>
            window.onload = function() {
                var bakiyeField = document.getElementById('bakiye');
                if (bakiyeField) {
                    bakiyeField.value = '$balance';
                }
            };
        </script>";
    } else {
...
<body>
    ...
    <center>
        <div class="container" style="width: 700px; height: 420px; background-color:rgb(182, 182, 182);">
            <h2>KentKart Bakiye Sorgulama </h2><br><br>
